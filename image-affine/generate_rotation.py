...
if __name__ == '__main__':
    ...
    #     affine.CustomizedRandomAffine(degree, choices, translate_range, f'./output_train/{name}_params.csv'),
    #     transforms.ToTensor(),
    #     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    # ])
    # save_to_file(datasets.CIFAR10(root='./data/cifar10', train=True, download=True, transform=rotation_transform),
    #              save_path=f'./output_train/{name}_rotation.pth')
    ...
    save_to_file(datasets.ImageFolder(root="~/tiny-imagenet/tiny-imagenet-200/train", transform=rotation_transform),
                 save_path=f'./output_train/{name}_rotation.pth')
